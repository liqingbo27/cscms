...
var home = {
        ...
        fans: function(uid,sid){
            var followBtns = $(".follow_btn"), otherUserIds = "";
            followBtns.each(function(){
                        ...
                        if(tmpFollorBtn && !isNaN(tmpUid) && tmpUid)
                        {
                            if(!!res1.data[tmpUid]){
                                if(res1.data[tmpUid] == 2){
                                    tmpFollorBtn.removeClass("watch_btn").addClass("other_btn").html("<i class=\"fa fa-exchange\"></i>");
                                }else{
                                    tmpFollorBtn.removeClass("watch_btn").addClass("has_btn").html("<i class=\"fa fa-check\"></i>");
                                }
                            }
                        }
                    });
              ...
              $.getJSON(cscms_path+"index.php/home/ajax/fans/"+uid+"/"+sid+"?random="+Math.random()+"&callback=?",function(data) {
                  if(data['error']=='ok'){ //关注成功
                       if(sid==2){
                           ...
                           $('.gz'+uid).html('<i class="fa fa-minus-square"></i>&nbsp;取消关注');
                       } else if(sid==1){
                           cscms.layer.msg('关注成功');
                           $('.gz'+uid).html('<i class="fa fa-exchange"></i>');
                       } else {
                           $('.gz'+uid).html("<i class=\"fa fa-minus-square\"></i>&nbsp;取消关注");
                       }
                  } else if(data['error']=='del'){ //解除成功
                       if(sid==2){
                           ...
                           cscms.layer.msg('您已经关注了对方',{icon:2});
                            ...
                            $('.gz'+uid).html("<i class=\"fa fa-plus-square\"></i>&nbsp;添加关注");
                       }
                  } else {
                       cscms.layer.msg(data['error'],{icon:2});
                  }
